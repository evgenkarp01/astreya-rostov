...
(function( $ ) {
			...
			function() {
				...
				$menu
					...
					.bind(
						evt( 'open' ),
						function( e ) {
							if ( opts.configuration.opened ) {return false;}
				...
				if ( opts.configuration.direction == 'horizontal' ) {
					...
					.bind(
						evt( 'toggle' ),
						function( e ) {return $(this).triggerHandler( evt( 'open' ) );}
						)
						.bind(
						evt( 'open' ),
						function( e ) {return openSubmenuHorizontal( $(this), opts );}
						)
						.bind(
						evt( 'close' ),
						function( e ) {return closeSubmenuHorizontal( $(this), opts );}
						);
				} else {
					$subs
					.bind(
				...
				if ( id && id.length ) {
					click( 'a[href="#' + id + '"]',
						function() {$menu.trigger( evt( 'toggle' ) );}
					);
				}
				var id = $page.attr( 'id' );
				if ( id && id.length ) {
					click( 'a[href="#' + id + '"]',
						function() {$menu.trigger( evt( 'close' ) );}
					);
				}

				// OPEN SUBMENU LINKS
				click( $( 'a.' + cls( 'subopen' ) + ', ' + 'a.' + cls( 'subclose' ), $menu ),
					function() {$( $(this).attr( 'href' ) ).trigger( evt( 'toggle' ) );}
				);
	...
	function _initMenu( $m, conf ) {
		if ( !$m.is( conf.menuNodetype ) ) {$m = $( '<' + conf.menuNodetype + ' />' ).append( $m );}
		...
		$( 'ul ul', $m )
			...
			.each(
				function( i ) {
					...
					if ( !$a.is( 'a' ) ) {$btn.addClass( cls( 'fullsubopen' ) );}
...
function cls( c ) {return 'mmenu-' + c;}
function dta( d ){return 'mmenu-' + d;}
function evt( e ) {return e + '.mmenu';}
})( jQuery );